...
impl<'a, W: io::Write> App<'a, W> {
    ...
    fn handle_request(&mut self, req: Request) -> Result<(), Error> {
        match &*req.method {
            ...
            "textDocument/didOpen" => {
                let params: lsp_types::DidOpenTextDocumentParams = serde_json::from_value(req.params)?;
            ...
            "textDocument/didChange" => {
                let params: lsp_types::DidChangeTextDocumentParams = serde_json::from_value(req.params)?;
            ...
            "textDocument/rename" => {
                ...
                self.send(&Response::success(req.id, lsp_types::WorkspaceEdit {
                    changes,
                    ..Default::default()
                }))?;
            }
    ...
    fn lookup_definition(&mut self, params: lsp_types::TextDocumentPositionParams) -> Option<lsp_types::Location> {
    ...
    fn completions(&mut self, params: lsp_types::TextDocumentPositionParams) -> Option<Vec<lsp_types::CompletionItem>> {
        let (ast, code) = self.files.get(&params.text_document.uri)?;
        let offset = utils::lookup_pos(code, params.position)?;
    ...
    fn rename(&mut self, params: lsp_types::RenameParams) -> Option<HashMap<Url, Vec<lsp_types::TextEdit>>> {
        let (ast, code) = self.files.get(&params.text_document.uri)?;
        let offset = utils::lookup_pos(code, params.position)?;
        ...
        fn rename_in_node(rename: &mut Rename, node: &Node) {
            if let Some(ident) = Ident::cast(node) {
                if ident.as_str() == rename.old {
                    rename.edits.push(lsp_types::TextEdit {
